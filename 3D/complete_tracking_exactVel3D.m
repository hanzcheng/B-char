function [newPoints,tTrack]=complete_tracking_exactVel3D(tstep,pointsToTrack,dv_exact,tTracking)
...
while ~all(time_leftB<=0)
        ...
        while ctrTstep<subTsteps
            newPoints(locToBTrack,:)=FO_Euler_exactVel3D(newPoints(locToBTrack,:),time_To_ExitB(locToBTrack),dv_exact,tTracking);
